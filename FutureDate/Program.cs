...
namespace FutureDate
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine($"{i}. {days[i]}"); //writing out a short menu to guide the user input
            }

            int currentDay = Convert.ToInt32(Console.ReadLine()); //collecting user input and convertingit to an integer

            if(currentDay < 0 || currentDay > days.Length -1){ //validating user input to make sure its a postive number and not more than the index of the array
                Console.WriteLine("Pick a valid day");
            }

            else{
                
                Console.WriteLine($"You picked {days[currentDay]}"); //displaying the day picked to the user
